...
const alphaSort = (arr, type) =>
  type === 'asc' || type === 'ascending' ? arr.sort((a, b) => a.localeCompare(b)) : arr.sort((a, b) => b.localeCompare(a));
...
describe('tests preparation', function () {
  ...
  describe('e2e tests', () => {
    it('load books properly', async () => {
      ...
      const pageData = await page.$$eval('tbody > tr > td', e =>
        e.map(x => x.innerText).filter(x => x.toLocaleLowerCase() !== 'edit' && x.toLocaleLowerCase() !== 'edit delete')
      );

      expect(alphaSort(pageData, 'asc').join('')).to.eq(alphaSort(jsonValuesToArray(JSON.stringify(mockData)), 'asc').join(''));
    });
  ...
  it(`testing delete functionality`, async () => {
    ...
    await page.route('**/jsonstore/collections/books/*', request => request.fulfill(json({ message: 'book deleted' })));
    await page.click('#loadBooks');
    ...
    const [response] = await Promise.all([
      page.waitForResponse(r => r.request().url().includes('/jsonstore/collections/books/') && r.request().method() === 'DELETE'),
  ...
  describe(`edit tests`, () => {
    ...
    it(`sends correct request`, async () => {
      ...
      const [response] = await Promise.all([
        page.waitForResponse(r => r.request().url().includes('/jsonstore/collections/books/') && r.request().method() === 'PUT'),
        page.click('text="Save"'),
      ]);
      const data = JSON.parse(await response.body());
