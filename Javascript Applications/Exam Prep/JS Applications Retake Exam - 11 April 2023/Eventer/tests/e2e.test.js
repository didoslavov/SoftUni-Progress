...
const endpoints = {
  ...
  own: (eventId, userId) => `/data/going?where=eventId%3D%22${eventId}%22%20and%20_ownerId%3D%22${userId}%22&count`,
};
...
describe('E2E tests', function () {
  ...
  describe('Navigation bar [ 5 Points ]', () => {
    ...
    it('Guest user should see correct navigation [ 2.5 Points ]', async () => {
      ...
      expect(await page.isVisible('nav >> text=Events'), 'Dashboard is not visible').to.be.true;
      expect(await page.isVisible('nav >> text=Add Event'), 'Create is visible').to.be.false;
  ...
  describe('Events Page [ 15 Points ]', () => {
    ...
    it('Check Events page with 2 events [ 2.5 Points ]', async () => {
      ...
      const names = await page.$$eval('.event .title', (t) => t.map((s) => s.textContent));
    ...
    it('Check Events page Info [ 5 Points ]', async () => {
      ...
      await page.waitForSelector('#dashboard');
      const names = await page.$$eval('.event .title', (t) => t.map((s) => s.textContent.trim()));
      const dates = await page.$$eval('.event  .date', (t) => t.map((s) => s.textContent.trim()));
  ...
  describe('CRUD [ 50 Points ]', () => {
    ...
    it('Check details information [ 5 Points ]', async () => {
      ...
      const date = await page.$$eval('#details-wrapper p #date', (t) => t.map((s) => s.textContent));
